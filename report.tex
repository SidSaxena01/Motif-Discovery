...
\subsection{Audio Stem Separation using Demucs}
Before proceeding with melody extraction, we applied a source separation step using Demucs (model: ht\_demucs\_ft). This advanced deep-learning model decomposes the audio tracks into four stems: vocals, bass, drums, and other. In our implementation, each audio file was processed with Demucs, and we observed that isolating the \textit{other} stem led to slightly improved melody extraction results. We hypothesize that by explicitly extracting the drums and bass stems, the algorithm is better able to focus on the relevant melodic content present in the remaining signal. This preprocessing step thus aids both the CREPE and Melodia models in achieving more accurate pitch contour extraction.
...
\textbf{CREPE:}
\begin{itemize}
    \item Utilizes a deep-learning framework to estimate the main pitch component from complex audio environments.
...
\begin{itemize}
    ...
    \item Although sensitive to noise and overlapping sounds, it can yield clearer pitch contours and is computationally efficient.
...
\bibitem{DeFossez2019} A. D{\'e}fossez, N. Usunier, L. Bottou, and F. R. Bach, ``Demucs: Deep Extractor for Music Sources with extra unlabeled data remixed,'' \textit{CoRR}, vol. abs/1909.01174, 2019.
\end{thebibliography}
...
\begin{figure}[htbp]
    ...
    \caption{Motif Detection via MASS.}
    \label{fig:mass}
\end{figure}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\linewidth]{figures/motif_detection_match.png}
    \caption{Motif Detection via MATCH.}
    \label{fig:match}
\end{figure}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\linewidth]{figures/motif_detection_stump.png}
    \caption{Motif Detection via STUMP.}
    \label{fig:stump}
\end{figure}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\linewidth]{figures/star_wars_motif_matches.png}
    \caption{Main Theme's Melody with Sections of Various Themes.}
    \label{fig:starwars}
\end{figure}
