...
More details about the code can be found in our GitHub repository\footnote{\url{https://github.com/SidSaxena01/Motif-Discovery}\label{foot:catalogue}}.
\end{abstract}
...
\begin{itemize}
    ...
    \item Compare traditional motif detection methods (using matrix profiles via stumpy\cite{law2019stumpy} and DTW) on the Star Wars dataset.
...
Previous studies have utilized both signal processing and deep learning approaches for motif detection. The matrix profile method has been successfully applied in exemplar works (e.g., \cite{Nuttall2023}) for discovering repeated segments in time series data. Alternative methods—such as techniques replacing DTW with string matching \cite{Ganguli2017}—demonstrate the value of symbolic abstraction in reducing computational complexity. More recent approaches have involved deep learning \cite{Krause2021} to detect leitmotif activities. Our work builds on these foundations by applying these methods to film soundtracks and addressing data synchronization challenges.
...
\begin{itemize}
    \item \textbf{Symbolic Data:} MusicXML files are parsed using \texttt{music21} to extract thematic elements such as motifs and phrases. The resulting note sequences are transformed into time-aligned arrays, with interpolation strategies ensuring precise synchronization with audio-derived features, such as the tempo.
...
\begin{itemize}
    \item \textbf{Stumpy (Matrix Profile):} We leverage the Python library \texttt{stumpy}\cite{law2019stumpy} to compute the matrix profile on the extracted pitch contours. The library enables rapid identification of candidate motifs. Hyper-parameters such as window length and exclusion zones were tuned experimentally.
    \item \textbf{Dynamic Time Warping (DTW):} To account for tempo variations and subtle interpretative differences in motif renditions, DTW is used for aligning time series data. Although this method provides precise alignment, it incurs higher computational complexity and is applied primarily as a refinement tool.
\end{itemize}

\noindent\textbf{Cross-Track Motif Comparison:}
To explore motif recurrence across various audio tracks, pitch contour segments are compared using DTW (Figure~\ref{fig:dtw-match}) and pairwise similarity measures (e.g., via \texttt{stumpy.mass}, Figure~\ref{fig:mass}). This analysis (Figure~\ref{fig:dtw-warp}) aims to uncover recurrent motifs that appear consistently across multiple tracks, offering insights into thematic connections within the dataset.
...
\begin{figure}[htbp]
    ...
    \caption{Motif detection using the MASS algorithm.}
    \label{fig:mass}
\end{figure}
...
Initial experiments indicate that Melodia demonstrates robustness in polyphonic contexts due to its deep-salience algorithm, which effectively identifies melody lines even when multiple instruments are present. In contrast, CREPE excels in providing clearer pitch contours for isolated segments. Based on these observations, we selected Melodia for subsequent motif detection experiments.

\subsection{Motif Detection Results}
Upon evaluating the results of our motif detection process, we estimate that approximately 80\% of the identified matches were accurate. This indicates that our method is yielding promising results, suggesting a strong potential for reliable motif detection in musical compositions.

\subsection{Cross-Track Motif Extraction}
Early experiments comparing motifs across different tracks have revealed encouraging patterns of recurrence. However, we recognize the need for further validation and refinement to enhance the reliability and accuracy of these findings.
...
We have not evaluated against the extracted time-stamps from the catalogue. This is because the catalogue \ref{foot:catalogue} only contains one time-stamp per movie appearance.

\subsection{Comparison of Methods}
Stumpy offers rapid candidate motif detection and computational efficiency—thanks to the well-optimized \texttt{stumpy} library—but is sensitive to parameter choices. In contrast, DTW yields more precise alignments at the expense of increased computational load. The trade-off between speed and accuracy is a key consideration in refining the pipeline.
...
\bibitem{FastestSimilaritySearch} A. Mueen, S. Zhong, Y. Zhu, M. Yeh, K. Kamgar, K. Viswanathan, C. Gupta, and E. Keogh, “The Fastest Similarity Search Algorithm for Time Series Subsequences under Euclidean Distance,” Aug. 2022. Available: \url{http://www.cs.unm.edu/~mueen/FastestSimilaritySearch.html}.
...
\bibitem{law2019stumpy} S. M. Law, ``STUMPY: A Powerful and Scalable Python Library for Time Series Data Mining,'' \textit{Journal of Open Source Software}, vol. 4, no. 39, p. 1504, 2019.
\bibitem{7837992} C.-C. Michael Yeh, Yan Zhu, Liudmila Ulanova, Nurjahan Begum, Yifei Ding, Hoang Anh Dau, Diego Furtado Silva, Abdullah Mueen, and Eamonn Keogh, ``Matrix Profile I: All Pairs Similarity Joins for Time Series: A Unifying View That Includes Motifs, Discords and Shapelets,'' in \textit{Proc. 2016 IEEE 16th International Conference on Data Mining (ICDM)}, 2016, pp. 1317--1322, doi:10.1109/ICDM.2016.0179.
\bibitem{7837898} Yan Zhu, Zachary Zimmerman, Nader Shakibay Senobari, Chin-Chia Michael Yeh, Gareth Funning, Abdullah Mueen, Philip Brisk, and Eamonn Keogh, ``Matrix Profile II: Exploiting a Novel Algorithm and GPUs to Break the One Hundred Million Barrier for Time Series Motifs and Joins,'' in \textit{Proc. 2016 IEEE 16th International Conference on Data Mining (ICDM)}, 2016, pp. 739--748, doi:10.1109/ICDM.2016.0085.
\bibitem{zhongMASSDistanceProfile2024} Sheng Zhong and Abdullah Mueen, ``{{MASS}}: Distance Profile of a Query over a Time Series,'' \textit{Data Mining and Knowledge Discovery}, vol. 38, no. 3, pp. 1466--1492, May 2024, doi:10.1007/s10618-024-01005-2.
\bibitem{8970797} Kaveh Kamgar, Shaghayegh Gharghabi, and Eamonn Keogh, ``Matrix Profile XV: Exploiting Time Series Consensus Motifs to Find Structure in Time Series Sets,'' in \textit{Proc. 2019 IEEE International Conference on Data Mining (ICDM)}, 2019, pp. 1156--1161, doi:10.1109/ICDM.2019.00140.
\bibitem{8215529} Chin-Chia Michael Yeh, Nickolas Kavantzas, and Eamonn Keogh, ``Matrix Profile VI: Meaningful Multidimensional Motif Discovery,'' in \textit{Proc. 2017 IEEE International Conference on Data Mining (ICDM)}, 2017, pp. 565--574, doi:10.1109/ICDM.2017.66.
\bibitem{inproceedings} Anh Dau and Eamonn Keogh, ``Matrix Profile V: A Generic Technique to Incorporate Domain Knowledge into Motif Discovery,'' in \textit{Proc. 2017 IEEE International Conference on Data Mining (ICDM)}, Aug. 2017, pp. 125--134, doi:10.1145/3097983.3097993.
\end{thebibliography}
